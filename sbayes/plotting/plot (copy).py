...
from sbayes.util import parse_area_columns, read_features_from_csv, sort_by_similarity
...
class Plot:
    ...
    @staticmethod
    def read_dictionary_vectorized(dataframe, search_key):
        param_dict = {}
        for column_name in dataframe.columns:
            if column_name.startswith(search_key):
                param_dict[column_name] = dataframe[column_name].to_numpy().astype(float)
    ...
    def read_simulation_stats(txt_path, dataframe):
        ...
        if 'ground_truth' in txt_path:
            true_posterior = dataframe['posterior'].to_numpy().astype(float)
            true_likelihood = dataframe['likelihood'].to_numpy().astype(float)
            true_prior = dataframe['prior'].to_numpy().astype(float)

            true_weights = Plot.read_dictionary_vectorized(dataframe, 'w_')
            true_alpha = Plot.read_dictionary_vectorized(dataframe, 'alpha_')
            true_beta = Plot.read_dictionary_vectorized(dataframe, 'beta_')
            true_gamma = Plot.read_dictionary_vectorized(dataframe, 'gamma_')

            recall, precision = None, None

        else:
            recall = dataframe['recall'].to_numpy().astype(int)
            precision = dataframe['precision'].to_numpy().astype(int)
    ...
    def read_stats(self, txt_path, simulation_flag):
        ...
        #
        posterior = df_stats['posterior'].to_numpy().astype(float)
        likelihood = df_stats['likelihood'].to_numpy().astype(float)
        prior = df_stats['prior'].to_numpy().astype(float)

        weights = Plot.read_dictionary_vectorized(df_stats, 'w_')
        alpha = Plot.read_dictionary_vectorized(df_stats, 'alpha_')
        beta = Plot.read_dictionary_vectorized(df_stats, 'beta_')
        gamma = Plot.read_dictionary_vectorized(df_stats, 'gamma_')
        posterior_single_areas = Plot.read_dictionary_vectorized(df_stats, 'post_')
        likelihood_single_areas = Plot.read_dictionary_vectorized(df_stats, 'lh_')
        prior_single_areas = Plot.read_dictionary_vectorized(df_stats, 'prior_')
    ...
    def transform_weights(self, feature, b_in, gt=False):
        ...
        if not gt:
            ...
            if len(inheritance_array) > 0:
                sample = np.column_stack([universal_array, contact_array, inheritance_array]).astype(np.float)
            else:
                sample = np.column_stack([universal_array, contact_array]).astype(np.float)
            return sample
...
# Sort weights by median contact
    ...
    def plot_weights(samples, feature, true_weights=False, labels=None, ax=None, mean_weights=False):
        ...
        plt.title(str(feature), loc='center', fontdict={'fontweight': 'bold', 'fontsize': 8})
        ...
        if true_weights:
            ...
            ax.scatter(*true_projected.T, color="#ed1696", lw=0, s=100, marker="*")

        if mean_weights:
            mean_projected = np.mean(samples, axis=0).dot(corners)
            ax.scatter(*mean_projected.T, color="#ed1696", lw=0, s=100, marker="o")

        if labels is not None:
            for xy, label in zip(corners, labels):
                xy *= 1.08  # Stretch, s.t. labels don't overlap with corners
                ax.text(*xy, label, ha='center', va='center', wrap=True,
                        fontdict={'fontsize': 8})

        ax.set_xlim(1.2*min(corners[:, 0]),
                    1.2*max(corners[:, 0]))
        ax.set_ylim(1.2*min(corners[:, 1]),
                    1.2*max(corners[:, 1]))
        ax.set_aspect('equal')
        ax.set_axis_off()
    ...
    def plot_probability_vectors(samples, feature, true_p=None, labels=None, ax=None, title=False):
        ...
        if n_p == 2:
            ...
            sns.distplot(x, rug=True, hist=False, color='teal',
                         kde_kws={'shade': True, 'lw': 0.4, 'clip': (0, 1), 'alpha': 0.15},
                         rug_kws={'color': 'k', 'alpha': 0.02, 'height': 0.03})
            ...
            if true_p is not None:
                ax.scatter(true_p[0], 0, color="#ed1696", lw=0, s=80, marker="*")

            if labels is not None:
                for x, label in enumerate(labels):
                    ax.text(x, -0.1, label, ha='center', va='top', wrap=True,
                            fontdict={'fontsize': 8})

            ax.plot([0, 0, 1, 1], [0.03, 0, 0, 0.03], c="k", lw=0.5)
            ax.set_xlim(-0.2, 1.2)
            ax.set_aspect(0.7)
            ax.set_axis_off()
            ...
            samples_ordered = samples[:, feat_order]

            # Project the samples
            samples_projected = samples_ordered.dot(corners)
            ...
            if true_p is not None:
                ...
                ax.scatter(*true_projected.T, color="#ed1696", lw=0, s=80, marker="*")
            ...
            if labels is not None:
                ...
                for xy, label in zip(corners, labels):
                    xy *= 1.08  # Stretch, s.t. labels don't overlap with corners
                    ax.text(*xy, label, ha='center', va='center', wrap=True,
                            fontdict={'fontsize': 8}, rotation=np.rad2deg(np.arctan2(xy[1], xy[0]))-90)
    ...
    def plot_weights_grid(self, fname, labels=None, burn_in=0.4):
        ...
        fig, axs = plt.subplots(n_row, n_col, figsize=(2 + 3 * n_col, 2 + 3 * n_row))
        position = 1
        ...
        fig.savefig(self.path_plots + fname, dpi=400, format="pdf", bbox_inches='tight')
