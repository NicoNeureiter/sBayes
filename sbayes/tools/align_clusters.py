...
def get_permuted_params(results: Results, permutation: list) -> pd.DataFrame:
    ...
    for i, j in enumerate(permutation):
        ...
        remap[f"size_cluster_{i}"] = params[f"size_cluster_{j}"]
...
def align_clusters(
    clusters_path_1: Path,
    clusters_path_2: Path,
    parameters_path_2: Path,
    clusters_path_2_out: Path,
    parameters_path_2_out: Path,
):
    # Load results
    clusters_1 = Results.read_clusters(clusters_path_1)
    results_2 = Results.from_csv_files(clusters_path_2, parameters_path_2, burn_in=0)
    clusters_2 = results_2.clusters

    # Compute the best permutation
    mean_clusters_1 = np.mean(clusters_1, axis=1)
    mean_clusters_2 = np.mean(clusters_2, axis=1)
    ...
    clusters_2_aligned = clusters_2[perm].transpose((1,0,2))
...
def main():
    ...
    parser.add_argument("--inplace", type=bool, nargs="?", default=False)
    ...
    if args.inplace:
        clusters_path_2_out = clusters_path_2
        parameters_path_2_out = parameters_path_2
    else:
