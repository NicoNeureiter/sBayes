    ...
    def _propose(
        ...
        **kwargs,
        ...
        if self.sample_from_prior:
            ...
            from_cluster = sample.source.value[..., 0] & sample.clusters.value[i_cluster, :, np.newaxis]
            features = from_cluster[..., np.newaxis] * self.get_likelihood(sample).features
        ...
        with sample.cluster_effect.edit_group(i_cluster) as cluster_effect:
            for i_feat in range(sample.n_features):
                ...
                cluster_effect[i_feat, s_idxs] = np.random.dirichlet(alpha=1 + feature_counts)
...
class GibbsSampleConfoundingEffects(Operator):
    ...
    def _propose(
        ...
        if self.sample_from_prior:
            ...
            from_group = (sample.source.value[:, :, self.source_index] &
        ...
        with sample.confounding_effects[conf].edit_group(i_group) as group_effect:
            for i_feat in range(sample.n_features):
                s_idxs = self.applicable_states[i_feat]
                feature_counts = np.nansum(features[:, i_feat, s_idxs], axis=0)
                group_effect[i_feat, s_idxs] = np.random.dirichlet(prior_counts[i_feat] + feature_counts)
...
class AlterClusterGibbsish(Operator):
    ...
    def _propose(self, sample: Sample, **kwargs) -> tuple[Sample, float, float]:
        if random.random() < 0.5:
            return self.gibbsish_shrink_cluster(sample)
        else:
            return self.gibbsish_grow_cluster(sample)

    def sample_sources(self, sample: Sample, site_subset: slice | NDArray[bool]):
        """Resample the observations to mixture components (their source).
        ...
        weights = update_weights(sample=sample)

        if self.sample_from_prior:
            source_posterior = weights[site_subset]
        else:
            ...
            source_posterior = normalize(
                lh_per_component[site_subset] * weights[site_subset], axis=-1
            )

        # Sample the new source assignments
        with sample.source.edit() as s:
            s[site_subset] = sample_categorical(p=source_posterior, binary_encoding=True)

        # The source probability may be needed for the hastings factor
        is_source = np.where(sample.source.value[site_subset].ravel())
        log_q = np.sum(np.log(source_posterior.ravel()[is_source]))

        return sample, log_q

    def calculate_current_source_prob(self, sample: Sample, site_subset=None) -> float:
        likelihood = self.model_by_chain[sample.chain].likelihood
        lh_per_component = likelihood.update_component_likelihoods(sample=sample)
        weights = update_weights(sample=sample)
        source_posterior = normalize(
        ...
        is_source = np.where(sample.source.value[site_subset].ravel())[0]
        return float(np.sum(np.log(source_posterior.ravel()[is_source])))
    ...
    def compute_cluster_posterior(
        ...
        if False:  # self.sample_from_prior:
            n_available = int(np.sum(available))
            p = np.random.random(n_available)
            return p
        ...
        has_components = deepcopy(sample.cache.has_components[available, :])
    ...
    def gibbsish_grow_cluster(self, sample: Sample) -> tuple[Sample, float, float]:
        # Choose a cluster
        ...
        neighbours = get_neighbours(cluster, occupied, self.adjacency_matrix)
        ...
        new_object = np.random.choice(sample.n_objects, p=p_add, replace=False)
        sample_new.clusters.add_object(i_cluster, new_object)
        ...
        log_q = np.log(p_add[new_object])
        log_q_back = np.log(p_remove[new_object])

        if model.sample_source and not self.sample_from_prior:
            # resample source
            sample_new, log_q_s = self.sample_sources(sample_new, site_subset=new_object)
            log_q_back_s = self.calculate_current_source_prob(
                sample, site_subset=new_object
            )

            # print(log_q_s, log_q_back_s)
            log_q += log_q_s
            log_q_back += log_q_back_s

        return sample_new, log_q, log_q_back

    def gibbsish_shrink_cluster(self, sample: Sample) -> tuple[Sample, float, float]:
        # Choose a cluster
        i_cluster = np.random.choice(range(sample.clusters.shape[0]))
        cluster = sample.clusters[i_cluster, :]
        ...
        # Draw new object according to posterior
        # print(p_remove)
        removed_object = np.random.choice(sample.n_objects, p=p_remove, replace=False)
        sample_new.clusters.remove_object(i_cluster, removed_object)
        ...
        log_q = np.log(p_remove[removed_object])
        log_q_back = np.log(p_add[removed_object])

        if model.sample_source and not self.sample_from_prior:
            # resample source
            sample_new, log_q_s = self.sample_sources(
                sample_new, site_subset=removed_object
            )
            log_q_back_s = self.calculate_current_source_prob(
                sample, site_subset=removed_object
            )
...
class AlterCluster(Operator):
    ...
    def get_likelihood_object(self, sample):
        return self.model_by_chain[sample.chain].likelihood
    ...
    def propose_new_sources(
        ...
        if MODE == 'gibbs':
            ...
            p = normalize(np.tile(has_components_new[changed_objects, None, :], (1, n_features, 1)))
            with sample_new.source.edit() as source:
                source[changed_objects, :, :] = sample_categorical(p, binary_encoding=True)
                log_q = np.log(p[source[changed_objects]]).sum()

            has_components_old = sample_old.cache.has_components.value
            p_back = normalize(np.tile(has_components_old[changed_objects, None, :], (1, n_features, 1)))
            log_q_back = np.log(p_back[sample_old.source.value[changed_objects]]).sum()
        else:
    ...
    def calculate_source_posterior(
        ...
        likelihood = self.get_likelihood_object(sample)
        lh_per_component = likelihood.update_component_likelihoods(sample)
    ...
    def swap_cluster(self, sample: Sample) -> tuple[Sample, float, float]:
        ...
        if self.resample_source:
            sample_new, log_q_s, log_q_back_s = self.propose_new_sources(sample, sample_new, [object_add, object_remove])
            log_q += log_q_s
            log_q_back += log_q_back_s
    ...
    def grow_cluster(self, sample: Sample) -> tuple[Sample, float, float]:
        ...
        if self.resample_source:
            sample_new, log_q_s, log_q_back_s = self.propose_new_sources(sample, sample_new, [object_add])
            log_q += log_q_s
            log_q_back += log_q_back_s
    ...
    def shrink_cluster(self, sample: Sample) -> tuple[Sample, float, float]:
        ...
        if self.resample_source:
            sample_new, log_q_s, log_q_back_s = self.propose_new_sources(sample, sample_new, [object_remove])
            log_q += log_q_s
            log_q_back += log_q_back_s
