...
from src.preprocessing import (get_p_global_prior, get_p_families_prior, compute_network,
...
# File location
TEST_SAMPLING_DIRECTORY = 'results/contact_zones/{experiment}/'.format(experiment=experiment_name)
TEST_SAMPLING_RESULTS_PATH = TEST_SAMPLING_DIRECTORY + 'sa_contact_zones_nz{nz}_{run}.pkl'
...
PRIOR = {'geo': {'type': "uniform", 'parameters': {}},
         ...
         'p_families': {'type': "dirichlet", 'parameters': {}}}
...
# p_global
p_global_dirichlet_sim, p_global_categories_sim = get_p_global_prior(feature_names=feature_names,
                                                                     category_names=category_names,
                                                                     file_location="data/p_global/",
                                                                     log=True)
...
# p_family
p_families_dirichlet, p_families_categories = get_p_families_prior(family_names=family_names,
                                                                   feature_names=feature_names,
                                                                   category_names=category_names,
                                                                   file_location="data/p_families",
                                                                   log=True)
PRIOR['p_families']['parameters']['dirichlet'] = p_families_dirichlet
PRIOR['p_families']['parameters']['categories'] = p_families_categories

# Estimate parameters for geo prior from the data
PRIOR['geo_prior_parameters'] = estimate_geo_prior_parameters(network, PRIOR['geo']['type'])
...
for run in range(N_RUNS):
    ...
    for N in range(1, N_ZONES + 1):
        ...
        zone_sampler = ZoneMCMC_generative(network=network, features=features,
                                           ...
                                           families=families, chain_swaps=N_SWAPS,
        ...
        # Use everything from the last run except for the weights
