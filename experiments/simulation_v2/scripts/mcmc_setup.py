...
class MCMCSetup:
    ...
    def logging_mcmc(self):
        ...
        logging.info("Pseudocounts for tuning the width of the dirichlet proposal distribution for weights: %s ",
                     self.config['mcmc']['PSEUDOC_DIR_PROPOSAL']['weights'])
        logging.info("Pseudocounts for tuning the width of the dirichlet proposal distribution for p_global: %s ",
                     self.config['mcmc']['PSEUDOC_DIR_PROPOSAL']['p_global'])
        logging.info("Pseudocounts for tuning the width of the dirichlet proposal distribution for p_zones: %s ",
                     self.config['mcmc']['PSEUDOC_DIR_PROPOSAL']['p_zones'])
        logging.info("Pseudocounts for tuning the width of the dirichlet proposal distribution for p_families: %s ",
                     self.config['mcmc']['PSEUDOC_DIR_PROPOSAL']['p_families'])
    ...
    def sampling_run(self, ops, initial_sample, **kwargs):
        if 'inheritance' in kwargs:
            ...
            inheritance_val = self.config['mcmc']['INHERITANCE']
        ...
        cur_zone_sampler = ZoneMCMC_generative(network=self.network, features=self.features,
                                               ...
                                               inheritance=inheritance_val,
                                               prior=self.config['mcmc']['PRIOR'],
                                               sample_p=self.config['mcmc']['SAMPLE_P'],
                                               var_proposal=self.config['mcmc']['PSEUDOC_DIR_PROPOSAL'])
    ...
    def save_stats(self, cur_run, cur_run_stats, **kwargs):
        if 'inheritance' in kwargs:
            inheritance_val = kwargs['inheritance']
        else:
            inheritance_val = self.config['mcmc']['INHERITANCE']
