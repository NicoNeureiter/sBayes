if __name__ == '__main__':
    ...
    TEST_SAMPLING_DIRECTORY = 'results/prior/prior_p_global/2019-10-25_21-07/'
    ...
    sample_path = TEST_SAMPLING_DIRECTORY + 'prior_p_global_pg' + str(pg) + '_' + str(run) + '.pkl'
    ...
    sites, site_names = get_sites("data/sites_simulation.csv", retrieve_subset=True)
    network = compute_network(sites=sites, subset=sites['subset'])

    # Plot posterior frequency
    plot_posterior_frequency(mcmc_res, net=network, nz=1, burn_in=0.4)
