...
TEST_SAMPLING_RESULTS_PATH = TEST_SAMPLING_DIRECTORY + 'parallel_zone_z_{z}_run_{run}.pkl'
    ...
    for run in range(N_RUNS):
        ...
        for N in range(N_ZONES):
            ...
            zone_sampler = ZoneMCMC(network=network, features=features,
                                    ...
                                    p_transition_mode=P_TRANSITION_MODE, n_zones=N+1,
            ...
            # Store the results
            path = TEST_SAMPLING_RESULTS_PATH.format(z=N+1, run=run)
            dump(run_stats, path)
